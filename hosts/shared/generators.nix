...
}:
{
  ...
  formatConfigs.digital-ocean = _config: {
    # If this is the first boot after install,
    # We need to get a SSH private key for the host, the pub keys are always present.
    # To prevent packaging the private key in the image, we need to query the key from the user interactively
